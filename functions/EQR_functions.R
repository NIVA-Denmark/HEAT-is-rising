#Owner: Ciarán J Murray
# read the list giving QE for each indicator
.QElist <- read.table(file="functions/indicatorQE.csv",sep=";",header=T,comment.char="", quote='"')
...
.HEATcategory <- function(QE){
  ...
  primary <- c("Phytoplankton","Planteplankton","Secchi")
  
  secondary <- c("Seagrass","Bottom fauna","Macroalgae","Oxygen",
                 "Ålegræs", "Bundfauna", "Makroalger")
...
#   #for not normalized scale:
#   ER<- ifelse(is.numeric(ER),ER,NA)
#   if(is.na(ER)){
#     return(NA_character_)
#   }else{
#     classid <- ifelse(ER>0.5,
#                       ifelse(ER>1,
#                              ifelse(ER>1.5,
#                                     ifelse(ER>2,1,2),
#                                     3),
